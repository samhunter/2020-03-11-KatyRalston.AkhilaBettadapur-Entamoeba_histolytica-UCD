...
# 2021-05-25 made axis.text.x and y size 14 instead of 12 and darkened the font from grey30 to grey20
ggplot(nucleotideFreq, aes(fill=Base, y=Freq, x=Sample)) +
    ...
    #theme(text = element_text(size=16)) + theme(legend.text=element_text(size=16)) +
...
ggplot(dtbncl, aes(x=niceNames, y=logReadCount, fill=Fragmentation)) +
    ...
    ggtitle("Log2 read count per fragment") + ylab('Log2 read count') + xlab('') +
    theme(text = element_text(size=15)) + theme(legend.text=element_text(size=15)) 
...
kpPlotRegions(kp, data.panel="ideogram", data=custom.ann, col=c("+"='black', "-"="darkgreen")[ann$V7])
kpAddBaseNumbers(kp, tick.dist=150000, add.units=T, cex=.9)
...
```{r p013_fragmentsPerGeneHeatmap, echo=FALSE, fig.height=16, fig.width=9, fig.align="center"}
# 
...
```{r p014_noCoverageGenesHeatmap, echo=FALSE, fig.height=16, fig.width=8, fig.align="center"}
