...
```{r p001_fragmentsPerSample, echo=FALSE, fig.height=3, fig.width=8, fig.align="center"}
library("gridExtra")
...
labs(title="Rarefaction curves by Library Type: Pilot Samples") + xlab("Sequenced Reads") + ylab('Fragments Detected')
```
...
geom_line(aes(color=Type)) +
geom_point(aes(color=Type)) +
labs(title="Rarefaction curves by Library Type, Final Samples") + xlab("Sequenced Reads") + ylab('Fragments Detected')
...
```{r p006_readsPerFragment, echo=FALSE, fig.height=6, fig.width=9, fig.align="center"}
...
kp = plotKaryotype(custom.genome, chromosomes='DS571145', zoom=zoom.region, plot.type=3, plot.params=plot.params, 
    main="Fragment coverage across 100kb region")
...
#kpPlotRegions(kp, data.panel=1, data=custom.Fcoverage, r0=0, r1=1, col='black', border='black', avoid.overlapping=T, num.layers=20)
#kpPlotRegions(kp, data.panel=2, data=custom.Rcoverage, r0=0, r1=1, col='darkgreen', border='darkgreen', avoid.overlapping=T, num.layers=20)
kpPlotCoverage(kp, data.panel=1, data=custom.Fcoverage, r0=0, r1=1, col='black', border='black', ymax=120)
kpPlotCoverage(kp, data.panel=2, data=custom.Rcoverage, r0=0, r1=1, col='darkgreen', border='darkgreen', ymax=120)
kpText(kp, chr="DS571145", y=0.5, x=0, data.panel="ideogram", col='black', label='Genes', cex=0.9, pos=2, clipping=F)
kpAddLabels(kp, labels="Forward\nFragments", r0=0, r1=.2, data.panel=1, col="black")
kpAddLabels(kp, labels="Reverse\nFragments", r0=0, r1=.2, data.panel=2, col="black")
kpAxis(kp, data.panel=1, ymin=0, ymax=120)
kpAxis(kp, data.panel=2, ymin=0, ymax=120)
...
kpPlotRegions(kp, data.panel="ideogram", data=custom.ann, col=c("+"='black', "-"="darkgreen")[ann$V7])
kpAddBaseNumbers(kp, tick.dist=1000, add.units=T)
# kpPlotRegions(kp, data.panel=1, data=custom.Fcoverage, r0=0, r1=1, col='black', border='black', avoid.overlapping=T, num.layers=20)
...
pilot.gr = GRanges(seqnames=pilotSDF$contig, 
                  ...
                  pilotSDF[,c('Pilot.gDNA','Pilot.Plasmid')])


pilot.gDNA.stats = calcCountStats(pilot.gr[pilot.gr$Pilot.gDNA>0])
pilot.Plasmid.stats = calcCountStats(pilot.gr[pilot.gr$Pilot.Plasmid>0])
...
final.gr = GRanges(seqnames=finalSDF$contig, 
                  ...
                  finalSDF[,c('Final.gDNA','Final.Plasmid')])

final.gDNA.stats = calcCountStats(final.gr[final.gr$Final.gDNA>0])
final.Plasmid.stats = calcCountStats(final.gr[final.gr$Final.Plasmid>0])


df = data.frame(Pilot.gDNA = pilot.gDNA.stats$stats, pilot.Plasmid = pilot.Plasmid.stats$stats,
                Final.gDNA = final.gDNA.stats$stats, Final.Plasmid = final.Plasmid.stats$stats)
...
```{r p013_fragmentsPerGeneHeatmap, echo=FALSE, fig.height=20, fig.width=8, fig.align="center"}
# 
...
pheatmap(log2(df+1), main='Log2 Fragment coverage by gene', show_rownames=F, ylab='Genes', 
        cluster_rows=F, cluster_col=F, scale='none',annotation_col=anncol, fontsize=7, gaps_col = seq(1, ncol(df)))
...
df = data.frame(Pilot.gDNA = sum(pilotSDF$Pilot.gDNA > 0),
                Pilot.Plasmid = sum(pilotSDF$Pilot.Plasmid>0),
                Final.gDNA = sum(finalSDF$Final.gDNA>0), 
                Final.Plasmid =sum(finalSDF$Final.Plasmid>0))
